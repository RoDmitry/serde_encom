//! A map of String to serde_encom::Value.
//!
//! By default the map is backed by a [`BTreeMap`]. Enable the `preserve_order`
//! feature of serde_encom to use [`IndexMap`] instead.
//!
...
/// #
/// # let mut map = serde_encom::Map::new();
/// # map.insert("key".to_owned(), serde_encom::Value::Null);
/// #
...
impl<'a> Entry<'a> {
    ...
    /// #
    /// let mut map = serde_encom::Map::new();
    /// map.entry("serde").or_insert(encom_from_json!(12));
    ///
    ...
    /// #
    /// let mut map = serde_encom::Map::new();
    /// map.entry("serde").or_insert_with(|| encom_from_json!("hoho"));
    ///
    ...
    /// let mut map = serde_encom::Map::new();
    /// map.entry("serde")
    ///     .and_modify(|e| *e = encom_from_json!("rust"))
    ///     .or_insert(encom_from_json!("cpp"));
    ///
...
impl<'a> VacantEntry<'a> {
    ...
    /// match map.entry("serde") {
    ///     Entry::Vacant(vacant) => {
    ///         vacant.insert(encom_from_json!("hoho"));
    ///     }
    ///     Entry::Occupied(_) => unimplemented!(),
    /// }
    /// ```
...
impl<'a> OccupiedEntry<'a> {
    ...
    ///
    /// let mut map = serde_encom::Map::new();
    /// map.insert("serde".to_owned(), encom_from_json!(12));
    ///
    /// match map.entry("serde") {
    ...
    ///
    /// let mut map = serde_encom::Map::new();
    /// map.insert("serde".to_owned(), encom_from_json!([1, 2, 3]));
    ///
    /// match map.entry("serde") {
    ///     Entry::Occupied(mut occupied) => {
    ///         occupied.get_mut().as_array_mut().unwrap().push(encom_from_json!(4));
    ///     }
    ///     Entry::Vacant(_) => unimplemented!(),
    /// }
    ///
    ...
    /// match map.entry("serde") {
    ///     Entry::Occupied(mut occupied) => {
    ///         occupied.into_mut().as_array_mut().unwrap().push(encom_from_json!(4));
    ///     }
    ///     Entry::Vacant(_) => unimplemented!(),
    /// }
    ///
    ...
    /// match map.entry("serde") {
    ///     Entry::Occupied(mut occupied) => {
    ///         assert_eq!(occupied.insert(encom_from_json!(13)), 12);
...
/// An iterator over a serde_encom::Map's entries.
pub struct Iter<'a> {
    iter: IterImpl<'a>,
}
...
/// A mutable iterator over a serde_encom::Map's entries.
...
/// An owning iterator over a serde_encom::Map's entries.
pub struct IntoIter {
    iter: IntoIterImpl,
}
...
/// An iterator over a serde_encom::Map's keys.
pub struct Keys<'a> {
    iter: KeysImpl<'a>,
}
...
/// An iterator over a serde_encom::Map's values.
pub struct Values<'a> {
    iter: ValuesImpl<'a>,
}
...
/// A mutable iterator over a serde_encom::Map's values.
