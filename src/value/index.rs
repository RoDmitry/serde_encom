        ...
        match v {
            Value::Array(vec) => {
                ...
                vec.get_mut(*self).unwrap_or_else(|| {
                    panic!(
                        "cannot access index {} of EnCom array of length {}",
                        self, len
                    )
                })
            }
            _ => panic!("cannot access index {} of EnCom {}", self, Type(v)),
        }
    }
}
...
impl Index for str {
    ...
    fn index_or_insert<'v>(&self, v: &'v mut Value) -> &'v mut Value {
        ...
        match v {
            ...
            _ => panic!("cannot access key {:?} in EnCom {}", self, Type(v)),
        }
    }
}
...
// loosey-goosey. You got some EnCom from an endpoint and you want to pull values
    ...
    I: Index,
{
    ...
    /// Index into a `serde_encom::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
    ...
    /// });
    ///
    /// assert_eq!(data["x"]["y"], encom_from_json!(["z", "zz"]));
    /// assert_eq!(data["x"]["y"][0], encom_from_json!("z"));
    ///
    /// assert_eq!(data["a"], encom_from_json!(null)); // returns null for undefined values
    /// assert_eq!(data["a"]["b"], encom_from_json!(null)); // does not panic
    /// ```
...
where
    I: Index,
{
    /// Write into a `serde_encom::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
    ...
    /// # use serde_encom::encom_from_json;
    /// #
    /// let mut data = encom_from_json!({ "x": 0 });
    ///
    /// // replace an existing key
    /// data["x"] = encom_from_json!(1);
    ///
    /// // insert a new key
    /// data["y"] = encom_from_json!([false, false, false]);
    ///
    /// // replace an array value
    /// data["y"][0] = encom_from_json!(true);
    ///
    /// // inserted a deeply nested key
    /// data["a"]["b"]["c"]["d"] = encom_from_json!(true);
    ///
    /// println!("{}", data);
    /// ```
